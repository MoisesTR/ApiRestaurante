...
module.exports = class BodegaSucursal {
    ...
    createBodega({IdSucursal, IdTipBode, NombBodega, DescLocal, Login, WorkSpace}) {
        const aoj = [];

        pushAOJParam(aoj, 'IdSucursal', sql.Int,        +IdSucursal);
        pushAOJParam(aoj, 'IdTipBode',  sql.TinyInt,    +IdTipBode);
        pushAOJParam(aoj, 'NumBodega',  sql.TinyInt,    NumBodega);
        pushAOJParam(aoj, 'NombBodega', sql.NVarChar(100),  NombBodega);
        pushAOJParam(aoj, 'DescLocal',  sql.NVarChar(200),  DescLocal);
        pushAOJParam(aoj, 'Login',      sql.NVarChar(150),  Login);
        pushAOJParam(aoj, 'WorkSpace',  sql.NVarChar(150),  WorkSpace);
    ...
    changeState({IdBodegaS, Habilitado}) {
        const aoj = [];
