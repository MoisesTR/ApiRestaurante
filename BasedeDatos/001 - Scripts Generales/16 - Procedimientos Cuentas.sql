...
AS BEGIN
	...
	BEGIN 
		...
		INSERT INTO CONTABILIDAD_CLASE_CUENTA(IdClasCuenta, NombClasC, DescClasC, Naturaleza)
		VALUES(@MAX_CUENTA, @NombClasC, @DescClasC, @Naturaleza)
...
GO
IF OBJECT_ID('USP_CREATE_CONTABILIDAD_SUBCUENTA','P') IS NOT NULL
	DROP PROCEDURE USP_CREATE_CONTABILIDAD_SUBCUENTA
GO
CREATE PROCEDURE USP_CREATE_CONTABILIDAD_SUBCUENTA (
	@NumCuenta				NVARCHAR(4),
...
AS BEGIN
	...
	IF ( @CANT_SUBCUENTAS < 99 ) BEGIN
		DECLARE @NUM_SUBCUENTA NVARCHAR(6);
		...
		SET @NumSubCuenta = @NUM_SUBCUENTA
	END
	ELSE BEGIN
